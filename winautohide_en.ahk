...
Menu, tray, Add, Require Ctrl Key, menuToggleCtrl
Menu, tray, Add, Unhide All Windows, menuUnautohideAll
Menu, tray, Add, Exit, menuExit
Menu, tray, Default, Settings...

; 根据配置设置菜单勾选状态
If (requireCtrl = 1) {
    Menu, tray, Check, Require Ctrl Key
} else {
    Menu, tray, Uncheck, Require Ctrl Key
}
...
createIndicator(winId, side) {
    ...
    ; 创建指示器GUI
    Gui, Indicator%winId%:New, +AlwaysOnTop -Caption +ToolWindow +LastFound, WinAutoHide Indicator
    ...
    ; 设置指示器提示信息
    WinGet, indicatorHwnd, ID, WinAutoHide Indicator
    if (indicatorHwnd) {
        ; 使用Windows API设置工具提示
        DllCall("SetWindowText", "Ptr", indicatorHwnd, "Str", "Hidden: " . winTitle)
    }
}
...
updateTrayTooltip:
    ...
    if (showTrayDetails) {
        ...
        tooltipText .= "Hidden Windows: " . hiddenCount
        
        if (hiddenCount > 0) {
            tooltipText .= "`n`nHidden Windows List:`n" . hiddenWindowsList
        }
        
        if (requireCtrl) {
            tooltipText .= "`n`nHold Ctrl key to show hidden windows"
        } else {
            tooltipText .= "`n`nMove mouse to edge to show hidden windows"
        }
        
        if (showIndicators) {
            tooltipText .= "`nEdge Indicators: Enabled"
        } else {
            tooltipText .= "`nEdge Indicators: Disabled"
        }
    } else {
        ; 简单模式：只显示程序名称
...
createSettingsGUI:
    ; 如果设置窗口已存在，则激活它
    IfWinExist, WinAutoHide Settings
{
    WinActivate, WinAutoHide Settings
    return
}
    
    ; 创建设置界面
    Gui, Settings:Add, Text, x20 y20 w300 h20, Basic Settings:
Gui, Settings:Add, Checkbox, x40 y50 w250 h20 vCtrlRequired gUpdateCtrlSetting, Require Ctrl key to show hidden windows
Gui, Settings:Add, Checkbox, x40 y80 w250 h20 vShowTrayDetails gUpdateTrayDetailsSetting, Show detailed info in tray tooltip
Gui, Settings:Add, Checkbox, x40 y110 w250 h20 vEnableDragHide gUpdateDragHideSetting, Enable drag-to-hide feature
Gui, Settings:Add, Checkbox, x40 y140 w250 h20 vShowIndicators gUpdateIndicatorsSetting, Show edge indicators
    
    ; 添加分隔线
    Gui, Settings:Add, Text, x20 y170 w300 h1 0x10 ; SS_ETCHEDHORZ
    
    ; 使用说明区域
    Gui, Settings:Add, Text, x20 y190 w300 h20, Usage Instructions:
    Gui, Settings:Add, Text, x40 y220 w280 h90, Use Ctrl+Arrow keys to hide current window to screen edge.`nMove mouse to edge to show hidden window.`nMoving a shown hidden window cancels auto-hide.`nWith drag-hide enabled, Ctrl+drag to edge also hides.`nEdge indicators show orange bars where windows are hidden.
    
    ; 按钮区域
    Gui, Settings:Add, Button, x40 y330 w80 h30 gShowAbout, About
Gui, Settings:Add, Button, x140 y330 w80 h30 gSaveSettings, Save
Gui, Settings:Add, Button, x240 y330 w80 h30 gCloseSettings, Close
    ...
    ; 显示设置窗口
    Gui, Settings:Show, w360 h380, WinAutoHide Settings
return
...
; 显示关于信息
 ShowAbout:
     MsgBox, 8256, About WinAutoHide, BoD winautohide v1.1 Modified Version`n`nOriginal Author: BoD (BoD@JRAF.org)`nModified by: hzhbest, MTpupil`nProject: https://github.com/MTpupil/winautohide`n`nMain Features:`n• Graphical settings interface`n• Detailed tray information`n• Drag-to-hide functionality`n• Edge indicators`n`nThis program and its source code are in the public domain.`nFor more information contact the original author BoD@JRAF.org
 return
 ...
 SaveSettings:
    ...
    ; 显示保存成功提醒（自动消失的Toast通知）
     ...
     Gui, Toast:Font, s10
     Gui, Toast:Add, Text, x15 y10 w120 h25 Center, Settings Saved!
